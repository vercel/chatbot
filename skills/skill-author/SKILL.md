...
type: capability
category: meta
runtime_type: orchestration_workflow
...
The skill-author capability enables Neptune to autonomously CREATE new skills/connectors, wire them into the file tree, update playbook markdown files, regenerate the master index, and ship to production via spawn_v2 handoff.
...
1. Agent loads this skill via `load_skill skills/skill-author`
2. Agent reads the playbook-skill-author.md for the matching routine
3. Agent follows the mandatory steps in order
4. Each step calls `execute_skill` pointing at the appropriate script
5. Final step: `spawn_v2` for commit → push → deploy → smoke tests

## Safety

- Scripts can ONLY touch connectors/_test_*, skills/_test_*, playbooks/_test_* OR connectors explicitly named in args
- No wildcard destruction
- No real customer connector edits (base44, nmi, slack, hyperswitch, vapi)
- All changes are committable with proper git history
