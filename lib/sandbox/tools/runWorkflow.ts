    ...
    steps: z
      .array(
        z.object({
          ...
          dependsOn: z.array(z.string()).optional().describe('Steps that must complete before this one'),
        })
      )
      .describe('Ordered list of workflow steps'),
    parallel: z.boolean().default(false).describe('Enable parallel fan-out for independent steps'),
