...
function routeLayer(req: ToolRequest): SandboxLayer {
  ...
  if (
    sessionKey &&
    ["runScript", "runWorkflow", "processData"].includes(tool)
  ) {
    return "persistent";
  }
  ...
  if (
    ["runScript", "scrapeURL", "processData", "generateMedia"].includes(tool)
  ) {
    return "ephemeral";
  }
  ...
  async execute(req: ToolRequest): Promise<OrchestrationResult> {
    ...
    switch (layer) {
      case "inline":
        return {
          layer,
          result: await this.executeInline(req),
      ...
      case "ephemeral":
        return {
          layer,
          result: await this.executeEphemeral(req),
          durationMs: Date.now() - startTime,
        };
      ...
      case "persistent":
        return {
          layer,
          result: await this.executePersistent(req),
          durationMs: Date.now() - startTime,
        };
      ...
      case "vps_bridge":
        return {
          layer,
          result: await this.executeVpsBridge(req),
          durationMs: Date.now() - startTime,
        };
      ...
      case "v2_handoff":
        return {
          layer,
          result: await this.executeV2Handoff(req),
          durationMs: Date.now() - startTime,
        };
    ...
    switch (tool) {
      case "runScript": {
        ...
        await sandbox.writeFiles([
          { path: filename, content: Buffer.from(code) },
        ]);
      ...
      case "scrapeURL": {
        const { url, selectors } = payload as {
          url: string;
          selectors?: string[];
        };
        const code = `
        ...
        `;
        await sandbox.writeFiles([
          { path: "/scrape.ts", content: Buffer.from(code) },
        ]);
      ...
      case "processData": {
        ...
        `;
        await sandbox.writeFiles([
          { path: "/process.ts", content: Buffer.from(code) },
        ]);
