...
pub(crate) async fn list(
    ...
    let threads = rustra.memory().list_threads(&principal.user_id, page.page()).await?;
    Ok(Json(threads))
}
...
pub(crate) async fn messages(
    ...
    rustra.memory().get_thread(&thread_id, &principal.user_id).await?;
    let messages =
        rustra.storage().recent_messages(&thread_id, query.limit.unwrap_or(100)).await?;
    Ok(Json(messages))
}
